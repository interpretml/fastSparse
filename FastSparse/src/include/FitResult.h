...
struct FitResult {
    ...
    arma::vec ExpyXB; // Used by Logistic regression, [p] <- ExpyXB[i] = exp(y_i * (x_i^T B + b0))
    arma::vec onemyxb; // Used by SquaredHinge regression

    arma::vec inverse_ExpyXB; // used by exponential loss for binary classification with binary features
};
