...
#' @return An S3 object of type "FastSparseCV" describing the regularization path. The object has the following members.
...
#' \item{fit}{The fitted model with type "FastSparse", i.e., this is the same object returned by \code{\link{FastSparse.fit}}.}
#'
#' @examples
...
#' fit <- FastSparse.cvfit(X, y, nFolds=5, seed=1, penalty="L0L2", maxSuppSize=20, nGamma=5,
...
#'
#' @export
FastSparse.cvfit <- function(x,y, loss="SquaredError", penalty="L0", algorithm="CD", 
                          ...
                          intercept=TRUE, lows=-Inf, highs=Inf)
{
    ...
    if (is(x, "sparseMatrix")){
        M <- .Call('_FastSparse_FastSparseCV_sparse', PACKAGE = 'FastSparse', x, y, loss, penalty,
        ...
        M <- .Call('_FastSparse_FastSparseCV_dense', PACKAGE = 'FastSparse', x, y, loss, penalty,
                   algorithm, maxSuppSize, nLambda, nGamma, gammaMax, gammaMin,
                   partialSort, maxIters, rtol, atol, activeSet, activeSetNum, maxSwaps, 
                   scaleDownFactor, screenSize, !autoLambda, lambdaGrid, nFolds, 
                   seed, excludeFirstK, intercept, withBounds, lows, highs)
